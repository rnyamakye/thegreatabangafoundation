...
function RelatedStories({
  ...
  useEffect(() => {
    const loadRelatedStories = async () => {
      try {
        ...
        const sameCategory = dataModule.impactStories
          .filter(
            (story: any) =>
              story.category === currentCategory && story.id !== currentStoryId
          )
          .slice(0, 2);

        // If we need more stories, get others from different categories
        const otherStories = dataModule.impactStories
          .filter(
            (story: any) =>
              story.category !== currentCategory && story.id !== currentStoryId
          )
  ...
  const getCategoryColor = (category: string) => {
    ...
    return colors[category] || "text-orange-600 bg-orange-100";
  };
  ...
  return (
    <div className="pt-8 mt-12 border-t border-gray-200">
      <h3 className="mb-6 text-2xl font-bold text-gray-900">Related Stories</h3>
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {relatedStories.map((relatedStory: any) => (
          <Link
            key={relatedStory.id}
            to={`/blog/${relatedStory.id}`}
            className="block overflow-hidden transition-transform duration-200 bg-white rounded-lg shadow-md hover:shadow-lg hover:scale-105"
          >
            <div className="relative h-48">
              {relatedStory.media.type === "image" && relatedStory.media.url ? (
                <img
                  src={relatedStory.media.url}
                  alt={relatedStory.media.alt}
              ...
              <div className="absolute top-3 left-3">
                <span
                  className={`px-2 py-1 text-xs font-semibold rounded-full ${getCategoryColor(relatedStory.category)}`}
                >
                  {relatedStory.category.replace("-", " ").toUpperCase()}
                </span>
              </div>
            </div>
            <div className="p-4">
...
export default function BlogDetail() {
  ...
  useEffect(() => {
    const loadStory = async () => {
      try {
        ...
        console.log(
          "Available stories:",
          dataModule.impactStories.map((s: any) => ({
            id: s.id,
            title: s.title,
          }))
        );
  ...
  const renderMedia = () => {
    if (story.media.type === "image" && story.media.url) {
      ...
      return (
        <div className="relative flex items-center justify-center rounded-lg shadow-lg h-96 bg-gradient-to-br from-blue-100 to-blue-200">
          <div className="relative">
            <svg
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-32 h-32 bg-white rounded-full bg-opacity-20"></div>
            </div>
          </div>
      ...
      <div className="max-w-5xl px-6 py-8 mx-auto">
        ...
        <div className="grid grid-cols-1 gap-8 mb-12 lg:grid-cols-2">
          ...
          <header className="order-2 lg:order-1">
            ...
            <h1 className="mb-4 text-3xl font-bold text-gray-900 lg:text-4xl xl:text-5xl">
              {story.title}
            </h1>
