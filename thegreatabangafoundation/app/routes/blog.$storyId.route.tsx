...
function RelatedStories({
  ...
  const getCategoryColor = (category: string) => {
    ...
    return colors[category.toLowerCase()] || "text-gray-600 bg-orange-100";
  };
          ...
          >
            <div className="relative h-48">
              ...
              <div className="absolute top-3 left-3">
                <span
                  className={`px-2 py-1 text-xs font-semibold rounded-full ${getCategoryColor(relatedStory._base)}`}
                >
                  {relatedStory._base.replace("-", " ").toUpperCase()}
                </span>
              </div>
            </div>
            <div className="p-4">
      ...
      <div className="max-w-5xl px-6 py-8 mx-auto">
        ...
        <div className="grid grid-cols-1 gap-8 mb-12 lg:grid-cols-2">
          ...
          <header className="order-2 lg:order-1">
            <div className="mb-4">
              <span
                className={`inline-block px-3 py-1 text-sm font-semibold rounded-full ${getCategoryColor(story._base)}`}
              >
                {story._base.replace("-", " ").toUpperCase()}
              </span>
            </div>
